...
create procedure pasarASiguienteEstacion(OUT nResultado INT, OUT cMensaje VARCHAR(500), in nro_chasis_input varchar(45)) 
	begin
        ...
        declare id_estacion_input INT DEFAULT 6; -- si con el select de debajo no cambia, significa que el nro de chasis corresponde a un vehiculo que ya salio de la ultima estacion
        ...
        if(id_estacion_input < 5) then -- si esta en una estacion que no es la ultima, le sumamos uno para chequear despues que no haya un auto del mismo modelo en esa.
			...
			if id_modeloAux>0 then
				if chasisOcupante is not null then
					...
					update vehiculo_x_estacion set fecha_hora_salida = current_time() where id_vehiculo = nro_chasis_input and id_estacion = id_estacion_input;
					update vehiculo_x_estacion set fecha_hora_entrada = current_time() where id_vehiculo = nro_chasis_input and id_estacion = (id_estacion_input+1); -- and id_estacion = nro_estacion_input;
